...
def dir_cnn():
    ...
    data_dir = "/Users/Amine/Desktop/Imperial College - CS/Spring term/Msc Project/Var-CNN--DynaFlow/preprocess"
    ...
    cnn_model, dilation_rate = dir_conv_block(cnn_input, 2, 4, dilation_rate) #2x2 kernel | 4 filters   #dilatation conv rate incremented inside blocks
    cnn_model, dilation_rate = dir_conv_block(cnn_model, 2, 8, dilation_rate) #2x2 kernel | 8 filters 
    cnn_model, dilation_rate = dir_conv_block(cnn_model, 2, 8, dilation_rate) #2x2 kernel | 8 filters 
    cnn_model, dilation_rate = dir_conv_block(cnn_model, 3, 16, dilation_rate) #3x3 kernel | 16 filters 
    cnn_model, dilation_rate = dir_conv_block(cnn_model, 3, 16, dilation_rate) #3x3 kernel | 16 filters 
    cnn_output = Flatten()(cnn_model)
    cnn_output = dense_layer(cnn_output, 1024, 0.4) #(1024=4094/4)? 
    ...
    predictions = model.predict(test_data[0], batch_size=50, verbose=1)
